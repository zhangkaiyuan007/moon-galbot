...
| 改动 | 文件 | 状态 | 作用 |
|---|---|---|---|
| 背景抹黑（点周围留一圈原图） | `tools/markers.py` + `training/train_act_pointonly.py` | ✅ 已改 | 抹掉背景纹理、保留物体局部，逼策略从点读位置 |
...
- **`MASK_BACKGROUND` 和 `KEEP_FRAC` 训练和部署必须一致**。部署 `markers.py` 与训练 `train_act_pointonly.py` 两个常量要相同值（`markers.py` 已默认 `True` / `0.055`）。
- **旧 checkpoint 不能配抹黑图部署**：旧 checkpoint 是"全背景图"训过的，抹黑部署会输入不匹配。想用旧 checkpoint 测就把 `MASK_BACKGROUND=False`。
